        ...
        <div class="group">
          <div class="groupItem" v-for="(itemInSLL, index) in itemInSL" :key="index">
            <span @click="goOther(itemInSLL)" class="software">
              ...
              <!-- 软件名称长度超出就显示提示框 -->
              <el-tooltip :disabled="itemInSLL.softwareName.length<29" :content="itemInSLL.name" placement="top" effect="light">
                <span class="softwareName">{{itemInSLL.softwareName}}</span>
              </el-tooltip>
            </span>
            <span class="star">
              <!-- TODO: 判断是否是监控中 -->
              <img src="../../assets/monitor.png" style="width: 14px;" v-if="itemInSLL.status === 0"/>
              <img src="../../assets/starOn.png" style="width: 14px;" v-if="itemInSLL.favorite" @click="addFrequentlyUsed(itemInSLL.id, itemInSLL.name, false)" />
              <img src="../../assets/starOff.png" style="width: 14px;" v-else @click="addFrequentlyUsed(itemInSLL.id, itemInSLL.name, true)" />
            </span>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    addFrequentlyUsed(id, exeName, isFrequentlyUsed){
      this.$http({
        url: this.$api.apiSoftwareAddFrequentlyUsed,
        method: 'POST',
        data: {
          id,
          isFrequentlyUsed,
