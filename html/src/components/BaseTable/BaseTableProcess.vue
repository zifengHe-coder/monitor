  ...
  <div id="baseTableCom">
    ...
    <div class="tableDiv" :style="tableStyle">
      <el-table
        ...
        stripe
        lazy
        :tree-props="{ children: 'children', hasChildren: 'isMultiple' }"
        :load="load"
  ...
  methods: {
    ...
    getCellWord(data, columnConfig) {
        if (
          ...
          if(data.row.isMultiple && columnConfig.prop == 'processId'){
            return "......"
          }else{
            return this.$utils.funcData.isNullOrUndefined(
            data.row[columnConfig.prop]
    ...
    load(tree, treeNode, resolve) {
      ...
      params.data.processName = tree.processName
      this.$http({
        url: this.$api.apiFileOperationSearchOneProcessData,
        method: "POST",
        data: {
          data:params.data
        },
